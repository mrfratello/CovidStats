...
export class PeriodOffset extends BaseChart {
  ...
  prepareDataset() {
    this.dataset = this.pureDataset.slice(this.offsetDays).map((item, i) => ({
      date: item.date,
      recoverDay: this.type === DYNAMIC_TYPE ? item.recoverDay : item.recover,
      deathsDay: this.type === DYNAMIC_TYPE ? item.deathsDay : item.deaths,
      casesDay:
        this.type === DYNAMIC_TYPE
  ...
  updateDomains() {
    this.maxCount =
      max(this.dataset.flatMap((item) => [item.casesDay, item.sumDay])) * 1.05
    this.countScale.domain([0, this.maxCount])
    this.timeScale.domain(this.dataset.map((item) => shortDate(item.date)))
    this.timeOffsetScale.domain(
      this.dataset.map((item) => shortDate(item.dateOffset)),
    )
  }
  ...
  updateRanges() {
    ...
    this.timeOffsetScale.range([this.marginLeft, this.width - this.marginRight])
  }

  renderAxes() {
    this.countAxis = axisRight().tickSizeOuter(0)
    this.countAxisBox = this.svg.append('g').classed('count_axis', true)

    this.timeAxesGroup = this.svg.append('g')

    this.timeAxis = axisBottom().tickSize(0).tickPadding(10)
    this.timeAxisBox = this.timeAxesGroup.append('g').classed('time_axis', true)

    this.timeOffsetAxis = axisBottom().tickSize(0).tickPadding(10)
  ...
  updateAxes() {
    ...
    this.timeOffsetAxisBox
      ...
      .attr(
        'transform',
        `translate(0, ${this.height - this.marginBottom + 30})`,
      )
  ...
  updateBars() {
    ...
    this.cases
      ...
      .attr(
        'height',
        (item) => this.countScale(0) - this.countScale(item.casesDay),
      )
    this.recover
      .selectAll('.recoverBar')
      .data(this.dataset, ({ date }) => date)
      ...
      .attr(
        'height',
        (item) => this.countScale(0) - this.countScale(item.recoverDay),
      )
    this.deaths
      .selectAll('.deathsBar')
      .data(this.dataset, ({ date }) => date)
      ...
      .attr(
        'height',
        (item) =>
          this.countScale(item.recoverDay) - this.countScale(item.sumDay),
      )
  }
